...
def shell_command(command: str, timeout_s: int) -> Tuple[str, str]:
    ...
    process = subprocess.run(
        command, timeout=timeout_s, capture_output=True, shell=True, check=True
    )
...
def main():
    ...
    for pi_group in pi_groups:
        ...
        preempt_usage = user_usage(
            accounts=[pi_group], partitions=["cpu-preempt", "gpu-preempt"], states=["running"]
        )
        ...
        for user, (preempt_cpus, preempt_gpus) in preempt_usage.items():
            ...
            overall_user_usage[user] = (
                running_cpus - preempt_cpus,
                running_gpus - preempt_gpus,
                0,
                0,
            )
